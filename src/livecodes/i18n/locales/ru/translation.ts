...
const translation: I18nTranslation = {
  about: {
    ...
    livecodes: {
      para1:
        '<1><2>LiveCodes</2></1> - это <3>проект с открытым исходным кодом</3>, <4>богатый функциями</4>, <5>клиентская</5> игровая площадка для кода. В настоящее время поддерживается <6>более 80 языков/<7></7>фреймворков</6>. Его можно использовать как отдельное приложение или <8>встраивать</8> в любую веб-страницу. Существует множество способов <9>предварительного заполнения площадок</9> кодом.',
      para2:
        'Широкий спектр <1>параметров конфигурации</1> делает его очень гибким. Мощный <2>SDK</2> (для <3>JS/TS</3>, <4>React</4>, <5>Vue</5> и <6>Svelte</6>) облегчает <7>встраивание</7> и <8>взаимодействие</8> с площадками. Доступна <9>исчерпывающая документация</9> с примерами кода, живыми демонстрациями и скриншотами.',
    },
    version: {
  ...
  editorSettings: {
    ...
    desc: 'Пожалуйста, ознакомьтесь с <1>документацией</1> для подробностей.',
    editor: {
      codejar: 'CodeJar',
  ...
  import: {
    ...
    code: {
      desc: 'Поддерживаемые источники: <1> <2>GitHub gist</2> <3>Файл GitHub</3> <4>Директория в репозитории GitHub</4> <5>Сниппет Gitlab</5> <6>Файл Gitlab</6> <7>Директория в репозитории Gitlab</7> <8>JS Bin</8> <9>Сырой код</9> <10>Код в DOM веб-страницы</10> <11>Код в zip-файле</11> <12>Официальные площадки<13></13>(TypeScript, Vue и Svelte)</12> </1> Пожалуйста, посетите <14>документацию</14> для подробностей.',
